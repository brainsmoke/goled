...
func (a *Fire2d) initColorTab() {
	...
	for i := 0; i < 64; i++ {
		...
		r := clip.FloatToByte(float64(i) * 12)
		g := clip.FloatToByte(math.Pow(float64(i)/64, 1.5) * 1024)
		b := clip.FloatToByte(float64(i*i) / 16)
...
func (a *Fire2d) initCaMap() {
	...
	for i := range a.caMap {
		a.caMap[i] = uintMin(511, uint(math.Pow(float64(i)/64, 1.25)/3.7*64.))
	}
}
...
func (a *Fire2d) Next() [][3]byte {
	...
	for y := 0; y < h; y++ {
		...
		a.cells[y*w] = a.caMap[uintMin(511, sum)]

		for x := 1; x < w-1; x++ {

			sum = a.cells[(y+1)*w+x-1] + a.cells[(y+1)*w+x] + a.cells[(y+1)*w+x+1]
			a.cells[y*w+x] = a.caMap[uintMin(511, sum)]

		}

		sum = a.cells[(y+2)*w-2] + a.cells[(y+2)*w-1] + a.cells[(y+1)*w]
		a.cells[y*w+w-1] = a.caMap[uintMin(511, sum)]
	}

	for x := 0; x < w; x++ {
		a.cells[h*w+x] = uint(rand.Intn(2)) * 56
	}
