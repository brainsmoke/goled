...
func NewShadowPlay(leds []model.Led3D, phaseMax, nColors int) *ShadowPlay {
	...
	for i := range leds {
		if leds[i].Inside {
			insideLeds = append(insideLeds, i)
		}
	}

	return &ShadowPlay{phaseMax: phaseMax, phase: 0, buf: buf, wave: wave, colors: colors, indices: indices, insideLeds: insideLeds}
}
...
func (t *ShadowPlay) Next() [][3]byte {
	...
	for i := range t.colors {
		...
		if p == 0 {
			t.indices[i] = t.insideLeds[rand.Intn(len(t.insideLeds))]
